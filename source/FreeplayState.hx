...
class FreeplayState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (songs[curSelected].songName.toLowerCase() == "old-splitathon") {
			if (curDifficulty != 1) {
				curDifficulty = 1;
				changeDiff();
			}
		}
		else if (songs[curSelected].week == 3 && (controls.LEFT_P || controls.RIGHT_P)) {
			if (curDifficulty == 1) curDifficulty = 3;
			else if (curDifficulty == 3) curDifficulty = 1;
			changeDiff();
		}
		else {
			if (controls.LEFT_P)
		...
		switch (songs[curSelected].week)
		{
			case 3:
				switch (curDifficulty)
				{
					...
					case 3:
						diffText.text = "EXTRA KEYS" + " - " + stupidBitch.toUpperCase();
				}
			default:
				switch (curDifficulty)
				{
	...
	function changeSelection(change:Int = 0)
	{
		...
		if (songs[curSelected].songName.toLowerCase() == "old-splitathon" && curDifficulty != 1) curDifficulty = 1;
		else if (songs[curSelected].week == 3 && curDifficulty != 1 && curDifficulty != 3) curDifficulty = 1;
